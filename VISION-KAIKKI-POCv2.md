...
**When `generate_summary` is false, use a template (still embedded, zero LLM cost):**

| Edge type | Template |
|---|---|
| `same_phenomenon` (cos ≥ 0.85) | `"{word1}" and "{word2}" are synonyms: {gloss1} / {gloss2}` |
| `extends` (cos ≥ 0.85) | `"{word1}" is derived from "{word2}"` |
| `applies_to` (cos ≥ 0.80) | `"{word1}" and "{word2}" share the etymon {root}` |
...
```js
// From a sense, climb to its concept cluster
db.barygraph.aggregate([
  { $match: { _id: senseNodeId } },
  { $graphLookup: {
      from: 'barygraph',
      startWith: '$_id',
...
| Build mongot vector index | ~4–8 hours | Yes |
| **Total to first queryable state** | **~6–10 hours** | |
...
| §2.3 — `v(type)` is embedding of edge type label | `v(type)` = embedding of a fixed natural-language sentence per type (e.g. `same_phenomenon` → *"these two words describe the same concept"*) | Bare labels embed poorly; fixed sentences avoid the circular dependency a centroid-of-edges approach would introduce (can't compute edge centroids before `bary_vec`, which needs `v(type)`) |
...
3. **Sparse L4–6.** Register/period tags (`archaic`, `slang`, `technical`) are absent on most kaikki senses. Levels 4–6 may be too thin to be useful — consider collapsing into L7 for the PoC and reinstating only if multi-language expansion needs them.
4. **Polysemy edge threshold.** Senses of the same headword are always connected, but at what `q`? Low cosine between "bank" (river) and "bank" (finance) is correct — they're distant. But `q` should still be high enough for the MetaBary scanner to pick them up.
...
|---|---|---|
| mongot HNSW build exhausts RAM at ~9M vectors | Medium | Build index on `doc_type: 'baryedge'` subset first to validate; fall back to fp16 vector quantization (halves footprint); last resort: offload `vector` field to Qdrant keyed by `_id`, keep graph in Mongo |
| Llama Scout Q4 summaries are low-quality / hallucinate | Low–Med | Spot-check first 1K before full run; prompt includes both glosses verbatim so model has no need to invent; reject outputs > 60 tokens or containing neither CM word |
| kaikki hypernym chains skip levels → broken one-step traversal | High | Stage 5 inserts synthetic intermediate synset nodes where `parent_level ≠ child_level − 1` |
| `bary_vec` averages to semantic mush (near-zero retrieval lift) | **This is the hypothesis under test** | If §8.1 shows no lift, re-run eval with `registry.summary` embedding as the primary BaryEdge vector instead of the algebraic barycenter — this decides CLAUDE.md open question §9.3 |
| Relation targets in kaikki point to words not in the English dump | Medium | Create lightweight stub nodes (`node_type: 'stub'`, no vector) so edges resolve; exclude stubs from eval |
| 3-day LLM stage interrupted | High | `pipeline_state` checkpoint every 10K edges (§6.3); stage 7 is append-only and idempotent |
